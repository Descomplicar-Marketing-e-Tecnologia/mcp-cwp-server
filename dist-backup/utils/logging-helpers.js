...
export function logToolEnd(context, result, error) {
...
// API call logging
export function logApiStart(method, url, data) {
    const context = {
        method,
        url,
        data,
...
export function logApiEnd(context, response, error, mode) {
    const duration = context.startTime ? Date.now() - context.startTime : 0;
    if (!isProduction) {
        if (mode === 'mock') {
...
export function logDataProcessing(step, input, output) {
    if (!isProduction) {
...
// Performance metrics
export function logPerformanceMetrics(operation, metrics) {
    if (!isProduction) {
...
export function logConfiguration(config) {
    if (!isProduction) {
