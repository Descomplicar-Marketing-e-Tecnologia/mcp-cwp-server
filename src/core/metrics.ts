...
export class MetricsCollector {
  ...
  private requestHistory: { timestamp: number; duration: number; success: boolean }[] = [];
  private maxHistorySize = 1000;
  
  // Performance thresholds
  private readonly SLOW_OPERATION_THRESHOLD = 2000; // 2 seconds
  private readonly HIGH_ERROR_RATE_THRESHOLD = 5; // 5%
  private readonly MEMORY_WARNING_THRESHOLD = 70; // 70%
  private readonly MEMORY_CRITICAL_THRESHOLD = 85; // 85%
  private readonly LOW_CACHE_HIT_RATE_THRESHOLD = 70; // 70%
   ...
   */
  getWorstPerformingTools(limit: number = 10): ToolMetrics[] {
    return Array.from(this.toolMetrics.values())
   ...
   */
  getHighestErrorRateTools(limit: number = 10): ToolMetrics[] {
    return Array.from(this.toolMetrics.values())
      .filter(metrics => metrics.total_calls >= 5)
   ...
   */
  private getRecentRequests(timeframeMs: number): typeof this.requestHistory {
   ...
   */
  private getCacheHitRateFromMetrics(): number {
