...
import {
  AutosslListArgsSchema,
  AutosslInstallArgsSchema,
  AutosslRenewArgsSchema,
  AutosslDeleteArgsSchema,
} from './types.js';

export async function handleAutosslTool(
  name: string,
  args: any,
  client: CwpClient
...
async function handleListSslCertificates(args: any, service: AutosslService): Promise<CallToolResult> {
  ...
  try {
    ...
    if (response.status === 'success') {
      ...
      if (validation.data!.domain) {
        filteredCerts = certificates.filter((cert: any) => 
          cert.ssl && cert.ssl.includes(validation.data!.domain!)
        );
      }
...
async function handleInstallSslCertificate(args: any, service: AutosslService): Promise<CallToolResult> {
...
async function handleDeleteSslCertificate(args: any, service: AutosslService): Promise<CallToolResult> {
...
async function handleRenewSslCertificate(args: any, service: AutosslService): Promise<CallToolResult> {
