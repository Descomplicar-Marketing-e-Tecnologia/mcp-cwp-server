...
import { safeStringEquals, safeConvertApiResponse } from '../../utils/type-safety.js';
...
export class AccountService {
  ...
  async createAccount(args: AccountCreateArgs): Promise<CwpResponse> {
  ...
  async updateAccount(args: AccountUpdateArgs): Promise<CwpResponse> {
    ...
    if (args.package !== undefined) data.package = String(args.package);
    if (args.email !== undefined) data.email = String(args.email);
    if (args.quota !== undefined) data.quota = String(args.quota);
    if (args.bandwidth !== undefined) data.bandwidth = String(args.bandwidth);
    if (args.autossl !== undefined) data.autossl = String(args.autossl ? '1' : '0');
    if (args.backup !== undefined) data.backup = String(args.backup ? '1' : '0');
  ...
  async deleteAccount(args: AccountDeleteArgs): Promise<CwpResponse> {
    const result = await this.client.post('/account', {
      action: 'del',
      username: args.username,
    });
  ...
  async suspendAccount(args: AccountSuspendArgs): Promise<CwpResponse> {
  ...
  async unsuspendAccount(args: AccountSuspendArgs): Promise<CwpResponse> {
    return await this.client.post('/account', {
      action: 'unsp',
      username: args.username,
    });
  ...
  async resetPassword(args: AccountPasswordResetArgs): Promise<CwpResponse> {
    return await this.client.post('/account', {
  ...
  async getAccountInfo(args: AccountInfoArgs): Promise<CwpResponse> {
    // Check cache first
    const cacheKey = cacheKeys.account(args.username);
    const cached = cache.get<CwpResponse>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  async listAccounts(args: AccountListArgs = {}): Promise<CwpResponse> {
    // Check cache first
