    ...
    try {
      ...
      if (isFeatureEnabled('health_checks')) {
        ...
        if (typeof healthConfig === 'object' && healthConfig && 'interval' in healthConfig) {
          healthChecker.startMonitoring(healthConfig.interval as number);
      ...
      logger.warn('Enterprise features initialization failed', { error: (error as Error).message });
    }
  }
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        const duration = Date.now() - startTime;
        if (isFeatureEnabled('performance_monitoring')) {
          metricsCollector.recordToolCall(name, duration, false, error as Error);
        }
        
        // Log tool call failure
        logToolEnd(toolContext, null, error as Error);
